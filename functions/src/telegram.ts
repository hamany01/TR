 ...
 * الـ Webhook الخاص بـ Telegram Bot لاستقبال الرسائل والتفاعلات من خوادم تيليجرام.
 */
export const telegramBotWebhook = functions.https.onRequest(async (req: any, res: any) => {
    ...
    if (message && message.text) {
      ...
      if (text.startsWith("/start")) {
        ...
        if (!token) {
          ...
          const introText = 
            `مرحباً بك في بوت منصة تذكير بالمواعيد الذكية! 🔔\n\n` +
          ...
          if (usersSnapshot.empty) {
            const errorText = 
              `⚠️ عذراً، رمز الربط هذا غير صالح أو منتهي الصلاحية.\n\n` +
            ...
            const successText = 
              `🎉 تم ربط حسابك بمنصة تذكير بمواعيدك بنجاح!\n\n` +
          ...
          if (eventData?.status === "completed") {
            if (chatId && messageId) {
              await editTelegramMessage(
                chatId,
                messageId,
                `✅ تم إنجاز هذا الحدث سابقاً بالفعل: *${eventData.title}*\n\nالمهام المسجلة دائماً في أمان.`
              );
            }
          } else {
            ...
            if (chatId && messageId) {
              const completedText = 
                `💯 رائع جداً! تم تدوين إنجاز المهمة وإيقاف التنبيهات القادمة لها:\n\n` +
