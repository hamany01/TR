...
export default function MyEvents({ userId, userProfile, onEditEvent, onAddNewEvent, refreshTrigger, onEventsLoaded }: MyEventsProps) {
  ...
  const [tgStatus, setTgStatus] = useState<{ [eventId: string]: { type: "success" | "error"; message: string } }>({});

  const sendTelegramReminder = async (ev: EventDoc) => {
    if (!userProfile?.telegramChatId) {
      setTgStatus(prev => ({
        ...prev,
        [ev.id!]: { type: "error", message: "يرجى تعيين Chat ID في صفحة الإعدادات أولاً للربط بالبوت!" }
      }));
      return;
    }
    ...
    if (!botToken) {
      setTgStatus(prev => ({
        ...prev,
        [ev.id!]: { type: "error", message: "⚠️ لم يتم العثور على توكن البوت VITE_TELEGRAM_BOT_TOKEN في Secrets المنصة." }
      }));
      return;
    }
    ...
    try {
      ...
      const formattedDate = `${eventDate.toLocaleDateString("ar-EG")} - ${eventDate.toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit" })}`;
      
      let text = `🔔 تذكير: ${ev.title}\n📅 الموعد: ${formattedDate}`;
      if (ev.notes) {
        text += `\n📝 ${ev.notes}`;
      }
      if (ev.link) {
        text += `\n🔗 الرابط المرتبط: ${ev.link}`;
      }
      ...
      if (response.ok && data.ok) {
        setTgStatus(prev => ({
          ...prev,
          [ev.id!]: { type: "success", message: "تم إرسال التذكير إلى تيليجرام بنجاح ✅" }
        }));
        setTimeout(() => {
          setTgStatus(prev => {
            const copy = { ...prev };
        ...
        throw new Error(data.description || "فشل إرسال الطلب إلى Telegram Bot API");
      }
    } catch (err: any) {
      console.error("Telegram API Error:", err);
      setTgStatus(prev => ({
        ...prev,
        [ev.id!]: { type: "error", message: `فشل الإرسال: ${err.message || err}` }
      }));
    } finally {
      setTgSending(null);
    }
                    ...
                    >
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 w-full sm:w-auto justify-end">
                          {ev.status === "active" && (
                            <button
                              onClick={() => sendTelegramReminder(ev)}
                              disabled={tgSending === ev.id}
                              className="px-3 py-2 bg-sky-50 hover:bg-sky-100 active:bg-sky-200 text-sky-700 disabled:bg-slate-100 disabled:text-slate-400 font-bold rounded-xl text-xs flex items-center gap-1 transition cursor-pointer border border-sky-200 shadow-sm"
                              title="إرسال تذكير مباشر إلى بوت تيليجرام"
                              id={`tg-send-btn-${ev.id}`}
                            >
                              {tgSending === ev.id ? (
                              ...
                              <span>إرسال تذكير تيليجرام الآن</span>
                            </button>
                          )}
                      ...
                      {tgStatus[ev.id!] && (
                        <div className={`p-2.5 rounded-xl text-xs font-bold leading-relaxed mb-3 text-center ${
                          tgStatus[ev.id!].type === "success" 
                            ? "bg-emerald-50 text-emerald-800 border border-emerald-100/55" 
                            : "bg-rose-50 text-rose-800 border border-rose-100/55"
                        }`}>
                          {tgStatus[ev.id!].message}
                        </div>
                      )}
